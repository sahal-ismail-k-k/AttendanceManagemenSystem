...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $name = $_POST['name'];
        $mobile = $_POST['mobile'];
        $num_subjects = intval($_POST['num_subjects']);
        
        // Fixed: Simpler subject collection
        $subjects = [];
        for ($i = 1; $i <= $num_subjects; $i++) {
            if (isset($_POST["subject_$i"]) && !empty(trim($_POST["subject_$i"]))) {
                $subjects[] = trim($_POST["subject_$i"]);
            }
        }
...
include 'db_connect.php';
        ...
        $stmt = $conn->prepare("INSERT INTO teachers (username, name, subjects, phone_number) VALUES (?, ?, ?, ?)");
        if (!$stmt->execute([$username, $name, $subjects_str, $mobile])) {
...
<div class="container">
    ...
    <div class="form-header">
        <p>Add new teachers to the system by filling out the form below</p>
    </div>
    ...
    <form id="teacherForm" method="POST">
        ...
        <div class="input-group">
            <label for="mobile">Mobile Number <span class="badge">Required</span></label>
            <div class="input-icon-wrapper">
                <input type="tel" id="mobile" name="mobile" placeholder=" " pattern="[0-9]{10}" required>
                <i class="fas fa-phone"></i>
                <span class="floating-label">Enter 10-digit mobile number</span>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    $('#teacherForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function() {
                showNotification('<i class="fas fa-exclamation-triangle"></i> An error occurred. Please try again.', 'error');
            },
            complete: function() {
                $('.loading').hide();
            }
